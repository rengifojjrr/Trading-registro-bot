...
- **Velas de**, dentro del gráfico: el tamaño de vela. Es cosa del dibujo y no de la medida, y mezclarlas era justo la confusión. Desde que el gráfico es el común, son las mismas nueve temporalidades que en una operación.
...
Luego fue un gráfico propio, con desplazamiento y zoom pero sin herramientas. Y ahora **es literalmente el mismo componente que el de una operación** (`components/charts/grafico.tsx`), con todo lo que eso trae: las veinte herramientas de dibujo, el imán, la regla, los indicadores, los tres modos de escala, el volumen, la reproducción vela a vela y la lista de dibujos con sus ajustes.

Eso no era una decisión de diseño que se haya cambiado de opinión: **los dibujos colgaban de una operación y de nada más** (`chart_drawings.trade_id`, `not null`), así que sobre las velas de un dato macro no había dónde guardar una línea. Resuelto eso en la tabla, mantener dos gráficos distintos habría sido mantener dos, y aquí se viene a hacer justo lo que se hace mirando un gráfico: medir el impulso, marcar el rango, trazar la línea que el precio acabó respetando.

Lo que separa a los dos cabe en `lib/charts/fuente.ts`, que es puro y está probado entero:

| | Operación | Publicación |
|---|---|---|
| Dibujos | `/api/trades/[id]/drawings` | `/api/noticias/[id]/drawings` |
| Velas | `/api/coinbase/trade-candles` (la ruta deriva la ventana) | `/api/economic-calendar/candles` (el tramo viaja en la URL) |
| Bordes | ventana fija, no hay nada que cargar | ±24 h / 48 h alrededor del dato |
| Encuadre inicial | que quepa todo | velas alrededor de la publicación |
| Marcas | entrada, salida, ejecuciones, plan | el dato, el final del plazo, el precio previo |

Y lo específico de cada uno viaja en dos props, `operacion` y `evento`. Lo que sólo tiene sentido con una operación detrás **no sale aquí**: la captura (se guarda *en* las capturas de una operación) y copiar los dibujos de la anterior (una publicación de hace un mes no tiene «la anterior»).

Lo demás que se aprendió por el camino, todo comprobado en un navegador de verdad con el componente real:

- **Abre en cinco minutos, no en uno.** Treinta velas antes y setenta después son hora y media de gráfico, y en hora y media de velas de un minuto no se aprecia la reacción a una noticia: se ve el ruido de dentro del movimiento, no el movimiento. En cinco minutos la misma cantidad de velas cubre ocho horas, así que entran los cuatro plazos que se miden y se ve la forma entera. No cuesta una petición: el servidor ya trae las velas de un minuto --las necesita para medir con precisión-- y de ellas salen las de cinco juntándolas en el cliente (`aggregateCandles`, alineadas con el reloj para que la vela del dato empiece exactamente a las 12:30).
- **Topes alrededor de la publicación** (24 h antes, 48 h después). Esto era **el gráfico corriéndose solo hacia la derecha al alejar el zoom**: el cargador del borde derecho pedía hasta *ahora mismo*, y para una noticia de junio eso son tres meses de velas disponibles. Alejar el zoom dejaba hueco, el cargador traía trescientas velas, seguía habiendo hueco, y vuelta a empezar. Medido con la misma lógica en un navegador: **18.361 velas sin tope contra 583 con él**. `tramoDelBorde` devuelve `null` en el tope, y eso es lo que corta el bucle.
- **Al fundir un tramo, la vista se queda donde estaba.** Éste era un segundo desplazamiento, distinto del anterior y que sólo se vio midiendo: `setData` conserva el rango *lógico* --los índices de las velas--, así que meter doscientas setenta y seis velas más antiguas por delante movía la vista doscientas setenta y seis velas hacia atrás. Se abría sobre la publicación y de pronto estabas mirando la madrugada anterior. Se vuelve a poner el rango de **tiempo**, y **dos veces**: la librería encola los datos y los vuelca en el repintado siguiente con su propio encuadre, que pisaba al primero. Sin la segunda vez, el gráfico acababa a veces sobre la publicación y a veces un día después, según qué llegara antes. Medido tres veces seguidas: ahora abre siempre en el mismo sitio.
- **Cuánto se pide al cambiar de temporalidad**: sesenta velas antes y doscientas cuarenta después, que son las trescientas de una petición. Pedir las setenta y dos horas enteras en velas de un minuto serían cuatro mil, la ruta recorta por el final, y el gráfico abriría enseñando **las cinco horas anteriores al dato** y ni una vela de la reacción.
- **«vs. previo» en la barra de datos**, detrás del OHLC de siempre: dónde está esa vela respecto al precio de justo antes del dato, que es la cifra que se viene a leer — la variación contra su propia apertura no dice nada aquí. Con su propio color, porque una vela roja dentro de un movimiento que sigue por encima del previo es verde en esa columna.
- **Marcas y referencia se ponen con los datos y no al crear el gráfico**, al contrario que la entrada y la salida de una operación. Es que sí cambian sin que cambie el gráfico: el plazo se elige con un botón, y el precio previo se recalcula cada vez que llega un tramo de velas anteriores. Puestas en la creación, cambiar de plazo reconstruiría el gráfico entero y le devolvería el zoom al sitio a quien lo estuviera mirando. Se **reutilizan** en vez de recrearse: creándolas de nuevo se apilaban, y el eje acababa con tres etiquetas «previo» superpuestas que además no eran el mismo precio.
- **Horas en tu zona** en el eje y en el crosshair: sin `tickMarkFormatter` la librería usa la del navegador y el gráfico contradiría a la tabla de al lado. Es una prop opcional del gráfico común, así que el de una operación sigue como estaba.
- **«Restablecer vista» es «volver al momento».** En una operación es «que quepa todo», que es lo que se quiere ver; aquí no, porque se han traído velas hasta dos días después del dato y enseñarlas todas deja la reacción convertida en un pellizco de tres píxeles. Reencuadra contando **velas** y no minutos: con minutos fijos, «los 45 minutos anteriores» es menos de una vela en la temporalidad de una hora.

La paleta sale de `lib/charts/tema-canvas.ts`, que ya era el sitio único donde se resuelven los tokens del tema para un canvas y de donde beben las gráficas de operaciones y de bots. Un canvas no entiende `var(--positive)`, así que hay que resolverlo en tiempo de ejecución; tenerlo en un solo módulo es lo que impide que dos gráficos de la misma aplicación acaben con dos verdes distintos.
