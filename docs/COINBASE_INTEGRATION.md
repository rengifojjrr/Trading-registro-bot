...
4. **Liquidación de contratos con vencimiento al expirar.** No está confirmado si el cierre forzado de una posición abierta al vencer un contrato `EXPIRING` llega como un `FILL` normal (que el motor manejaría automáticamente) o como algún registro de liquidación fuera de banda que el motor no vería como fill. Debe confirmarse antes de confiar en el motor para un contrato que realmente expiró con posición abierta.
