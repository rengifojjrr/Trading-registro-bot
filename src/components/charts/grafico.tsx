...
/**
 * El cierre de la última vela anterior a la publicación.
 *
 * Es la referencia de toda la lectura de una noticia: «subió un 1,2 %» sólo
 * significa algo contra el precio que había justo antes de que saliera el dato,
 * no contra la apertura de la vela que uno esté mirando.
 */
function precioAntesDe(velas: TradeChartCandle[], t0: number): number | null {
...
/**
 * El gráfico de velas de la aplicación. Uno solo, para las dos cosas que se
 * miran en velas: una operación tuya y la reacción a una publicación macro.
 *
 * Construido sobre lightweight-charts, la librería abierta de la propia
 * TradingView, servida desde aquí y sin iframe: el widget de TradingView no
 * sabe saltar de forma fiable a una ventana histórica concreta y seguramente
 * ni siquiera tenga símbolo para este contrato de futuros. Es ilustrativo:
 * nunca es la fuente de verdad de un P&L ni de ninguna otra cifra de la página.
 *
 * **Servía sólo para operaciones, y ése era el problema.** El gráfico de una
 * publicación era otro componente, y no tenía herramientas de dibujo, ni
 * indicadores, ni escalas, ni reproducción. No por decisión de diseño: los
 * dibujos colgaban de una operación y de nada más, así que sobre las velas de
 * un dato macro no había dónde guardar una línea. Un gráfico sin herramientas
 * al lado de otro que sí las tiene no se lee como «más sencillo», se lee como
 * roto.
 *
 * Lo que cambia entre los dos cabe en `lib/charts/fuente.ts`: de quién son los
 * dibujos, de dónde salen las velas y hasta dónde se traen. Y luego lo que sólo
 * tiene cada uno -- entrada, salida y plan en una operación; el precio previo y
 * el plazo medido en una publicación --, que viaja en `operacion` y `evento`.
 *
 * Las herramientas de marcado se guardan en `chart_drawings` y se pintan encima
 * de las velas: las líneas horizontales reutilizan el mismo `createPriceLine`
 * que la entrada y el stop; el resto va en un `<canvas>` aparte, colocado
 * encima en posición absoluta y repintado desde {tiempo, precio} en cada
 * desplazamiento, zoom y cambio de tamaño con los propios conversores de
 * coordenadas de la librería. (Su API de primitivas también sirve, pero pide su
 * propio destino de dibujo; un lienzo suelto es el camino simple y
 * igual de correcto para un puñado de formas.)
 */
/**
 * La vista guardada de este gráfico, o la de fábrica.
 *
 ...
 */
function leerVista(clave: string, porDefecto: string): ChartViewState {
  try {
...
function guardarVista(clave: string, vista: ChartViewState): void {
  try {
    window.localStorage.setItem(clave, JSON.stringify(vista));
  } catch {
    // Sin sitio o sin permiso: se pierde la preferencia, no la sesión.
  }
}

/** Lo que sólo tiene una operación: por dónde entraste, por dónde saliste, tu plan. */
export interface DatosDeOperacion {
  /**
   * Hacia dónde apuntan las flechas de entrada y salida. Entrar en corto es
   * vender, así que una flecha hacia arriba bajo la vela diría lo contrario de
   * lo que pasó.
   */
  ...
  /** Sólo una posición abierta tiene un «precio ahora mismo» que dibujar. */
  isOpen?: boolean;
  /**
   * Del diario: el nivel que tú planeaste, no algo que informe Coinbase. Se
   * pinta como raya discontinua y se puede arrastrar.
   */
  stopLoss?: number | null;
  takeProfit?: number | null;
}

/** Lo que sólo tiene una publicación macro. */
export interface DatosDeEvento {
  /** El plazo que miden las cifras de al lado: se marca en el gráfico. */
...
export function Grafico({
  ...
  /**
   * Dos props opcionales y no una unión discriminada por `fuente.tipo`, que
   * sería lo estricto: TypeScript no estrecha una unión por una propiedad
   * anidada, así que la unión obligaría a repetir el discriminante arriba sólo
   * para que el compilador lo viera. Quien monta el gráfico es una de las dos
   * envolturas de al lado, y cada una pasa lo suyo.
   */
  operacion?: DatosDeOperacion | null;
  evento?: DatosDeEvento | null;
  /**
   * La zona en la que se leen las horas del eje.
   *
   * Opcional: sin ella la librería usa la del navegador, que es lo que el
   * gráfico de una operación ha hecho siempre. Donde hace falta es al lado de
   * una tabla de horas -- la de una publicación macro --, porque dos relojes
   * distintos en la misma pantalla convierten comparar en descifrar.
   */
  timezone?: string;
}) {
  const esOperacion = fuente.tipo === "operacion";
  const direction = operacion?.direction ?? "LONG";
  const entry = operacion?.entry ?? null;
  const exit = operacion?.exit ?? null;
  const fills = operacion?.fills;
  const isOpen = operacion?.isOpen ?? false;
  const stopLoss = operacion?.stopLoss ?? null;
  const takeProfit = operacion?.takeProfit ?? null;
  // Los extremos, sacados aquí y no dentro de los efectos: TypeScript no
  // conserva el estrechamiento de `fuente` dentro de una función anidada.
  const desdeOperacion = fuente.tipo === "operacion" ? fuente.desde : null;
  const hastaOperacion = fuente.tipo === "operacion" ? fuente.hasta : null;
  /** El instante de la publicación. La referencia contra la que se mide todo. */
  const t0 = fuente.tipo === "evento" ? fuente.t0 : null;
  const claveVista = claveDeVista(fuente);
  const rutaDibujos = rutaDeDibujos(fuente);

  const enLaZona = useCallback(
    (t: number, patron: string) =>
      DateTime.fromSeconds(t, { zone: "utc" })
  ...
  /**
   * Las marcas y la referencia de una publicación.
   *
   * Se reutilizan en vez de crearse otra vez con cada tramo de velas: crear una
   * capa de marcadores nueva la apila sobre la anterior, y una línea de precio
   * nueva deja la vieja puesta en el eje.
   */
  const marcadoresRef = useRef<ISeriesMarkersPluginApi<Time> | null>(null);
  const lineaPrevioRef = useRef<IPriceLine | null>(null);
   ...
   */
  const vistaGuardada = useState(() => leerVista(claveVista, initialGranularity))[0];
  ...
  const showsWholeTrade = useMemo(
    () =>
      ...
      hastaOperacion === null ||
      coversWholeTrade(
        new Date(desdeOperacion * 1000),
        new Date(hastaOperacion * 1000),
        granularity,
      ),
    [desdeOperacion, hastaOperacion, granularity],
  );
  ...
  /**
   * Volver al encuadre de salida.
   *
   * En una operación es «que quepa todo», que es lo que se quiere ver. En una
   * publicación no: se han traído velas hasta dos días después del dato y
   * enseñarlas todas deja la reacción convertida en un pellizco de tres
   * píxeles. Ahí «restablecer» es volver al momento.
   */
  function resetView() {
    ...
    const encuadre = encuadreInicial(
      t0,
      GRANULARITY_SECONDS[granularity],
      candlesRef.current.map((c) => c.time),
    );
    escala.setVisibleRange({
      from: encuadre.from as UTCTimestamp,
      to: encuadre.to as UTCTimestamp,
    });
  }
  ...
  async function captureChart() {
    ...
    try {
      ...
      formData.set("tradeId", fuente.id);
  ...
  async function updateDrawingStyle(id: string, style: DrawingStyle) {
    ...
    try {
      const response = await fetch(rutaDeDibujos(fuente, id), {
        method: "PATCH",
  ...
  /**
   * Si hay un tramo en vuelo.
   *
   * Por referencia y no por estado: lo lee el manejador del desplazamiento, que
   * se registra una vez al crear el gráfico. Sin él, arrastrar hasta el borde
   * dispara veinte peticiones del mismo tramo.
   */
  const cargandoTramoRef = useRef(false);
  /** El fotograma en el que se vuelve a poner la vista tras fundir un tramo. */
  const reencuadreRef = useRef(0);

  /**
   * Trae las velas de una temporalidad y sustituye las que había.
   *
   * Lo comparten el selector de temporalidad y el refresco de fondo, para que
   * los dos pasen por el mismo sitio. `silent` es lo que evita que el refresco
   * de cada minuto encienda el indicador de carga o se queje de un tropiezo de
   * red: lo volverá a intentar dentro de un minuto.
   *
   * Qué URL toca lo decide `fuente`, no un `if` aquí: la de una operación
   * deriva la ventana de las marcas de la propia operación, la de una
   * publicación la lleva escrita.
   */
  async function loadCandles(nextGranularity: CoinbaseCandleGranularity, silent = false) {
    ...
    try {
      const res = await fetch(
        peticionDeVelas(fuente, productId, nextGranularity, GRANULARITY_SECONDS[nextGranularity]),
      );
  ...
  /**
   * Trae un tramo de al lado y lo funde con lo que ya hay.
   *
   * Sólo para una publicación: es lo que convierte «la ventana que decidió la
   * aplicación» en «mira lo que quieras» -- hacia atrás para ver qué venía
   * haciendo el precio esa mañana, hacia delante para ver si el movimiento
   * aguantó. Una operación tiene ventana fija y aquí no entra nunca.
   */
  async function traerTramo(desde: number, hasta: number) {
    if (cargandoTramoRef.current) return;
    cargandoTramoRef.current = true;
    try {
      const g = granularityRef.current;
      const res = await fetch(
        peticionDeVelas(fuente, productId, g, GRANULARITY_SECONDS[g], { desde, hasta }),
      );
      const data = (await res.json()) as { candles: TradeChartCandle[] | null };
      ...
      // Sin red se queda lo que ya estaba dibujado, que es lo correcto: un
      // gráfico es contexto, no una cifra de la que dependa nada.
    } finally {
      cargandoTramoRef.current = false;
    }
  }
  ...
  async function saveDrawing(tool: ToolId, points: DrawingPoint[]) {
    ...
    try {
      const response = await fetch(rutaDibujos, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function persistPlanLevel(cual: "stop" | "target", precio: number | null) {
    try {
      const res = await fetch(`/api/trades/${fuente.id}/plan`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function persistMovedDrawing(drawing: TradeChartDrawing) {
    try {
      const res = await fetch(rutaDeDibujos(fuente, drawing.id), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDeleteDrawing(id: string) {
    ...
    try {
      const res = await fetch(rutaDeDibujos(fuente, id), { method: "DELETE" });
      const result = (await res.json()) as { error: string | null };
      if (result.error) toast.error(result.error);
    } catch {
  ...
  async function handleImportDrawings() {
    setImporting(true);
    try {
      const res = await fetch(`/api/trades/${fuente.id}/drawings/import`, { method: "POST" });
  ...
  async function handleClearDrawings() {
    ...
    const results = await Promise.all(
      previous.map((d) =>
        fetch(rutaDeDibujos(fuente, d.id), { method: "DELETE" })
          .then((r) => r.ok)
          .catch(() => false),
      ),
    );
  ...
  useEffect(() => {
    ...
    const chart: IChartApi = createChart(container, {
      ...
      timeScale: {
        ...
        // Las horas del eje, en la zona configurada. Sin esto la librería las
        // pinta en la del navegador, y el gráfico contradiría a la tabla que
        // tiene al lado -- que es justo la comparación que se viene a hacer.
        ...(timezone ? { tickMarkFormatter: (t: Time) => enLaZona(t as number, "HH:mm") } : {}),
      },
      ...
      localization: {
        locale: "en-US",
        ...(timezone
          ? { timeFormatter: (t: Time) => enLaZona(t as number, "dd LLL HH:mm") }
          : {}),
      },
    });
    ...
    if (!entry) {
      // Una publicación macro no marca nada aquí: sus dos flechas y su línea de
      // referencia se ponen en el efecto de los datos, unas líneas más abajo.
      // El motivo es el plazo que se mide, que cambia al pulsar «Medir a»; si
      // se pintaran aquí, cambiar de plazo reconstruiría el gráfico entero y le
      // devolvería el zoom al sitio a quien lo estuviera mirando.
    } else if (fills && fills.length > 0) {
    ...
    if (entry) createSeriesMarkers(series, markers);
    ...
    function updateLegend(param: MouseEventParams<Time>) {
      ...
      const campos: [string, string, string | null][] = [
        ["A", formatMoney(shown.open), null],
        ["Máx", formatMoney(shown.high), null],
      ...
      // esta vela sino dónde está respecto al precio de justo antes del dato.
      // Va la última, detrás de la lectura de siempre, porque es la respuesta y
      // no el contexto. Y con su propio color: una vela roja dentro de un
      // movimiento que sigue por encima del previo es verde en esta columna.
      if (t0 !== null) {
        const previo = precioAntesDe(current, t0);
        if (previo !== null && previo !== 0) {
          const desdeElDato = ((shown.close - previo) / previo) * 100;
          campos.push([
            "vs. previo",
            `${desdeElDato >= 0 ? "+" : ""}${desdeElDato.toFixed(2)}%`,
            desdeElDato >= 0 ? THEME.up : THEME.down,
          ]);
        }
      }

      node.replaceChildren(
        ...campos.map(([etiqueta, valor, colorDelDato]) => {
          ...
          if (colorDelDato) dato.style.color = colorDelDato;
    ...
    /**
     * Al llegar cerca de un borde, pedir el tramo de al lado.
     *
     * Sólo donde hay bordes que perseguir, que es una publicación: `topesDeVelas`
     * devuelve `null` para una operación, cuya ventana la fija la propia
     * operación y no se amplía. Los dos topes son de la publicación y no del
     * reloj -- sin ellos, alejar el zoom sobre una noticia de hace tres meses
     * dejaba hueco a la derecha, el cargador traía trescientas velas, seguía
     * habiendo hueco, y el gráfico se corría solo mientras lo mirabas.
     */
    function onLogicalRange() {
      const topes = topesDeVelas(fuente, Math.floor(Date.now() / 1000));
      if (!topes || cargandoTramoRef.current) return;

      const rango = chart.timeScale().getVisibleLogicalRange();
      const actuales = candlesRef.current;
      if (!rango || actuales.length === 0) return;

      const tiempos = actuales.map((c) => c.time);
      const paso = GRANULARITY_SECONDS[granularityRef.current];

      const lado =
        rango.from < MARGEN_VELAS
          ? "izquierda"
          : rango.to > actuales.length - MARGEN_VELAS
            ? "derecha"
            : null;
      if (!lado) return;

      const tramo = tramoDelBorde(lado, tiempos, paso, topes);
      if (tramo) void traerTramo(tramo.desde, tramo.hasta);
    }
    ...
    return () => {
      ...
      // que el siguiente intentara quitar una línea de una serie muerta.
      marcadoresRef.current = null;
      lineaPrevioRef.current = null;
      cancelAnimationFrame(reencuadreRef.current);
  ...
  useEffect(() => {
    ...
    /**
     * Lo que se estaba mirando, en tiempo, antes de tocar los datos.
     *
     * **Aquí estaba el gráfico moviéndose solo**, y no era el mismo fallo que
     * el de los topes. `setData` conserva el rango *lógico* -- los índices de
     * las velas --, así que al fundir doscientas velas más antiguas por
     * delante, los mismos índices pasan a señalar otro momento y la vista
     * retrocede doscientas velas sin que nadie la haya tocado. Se abría sobre
     * la publicación, llegaba el tramo de la izquierda, y de pronto estabas
     * mirando la madrugada anterior.
     *
     * Se vuelve a poner el rango de tiempo más abajo, salvo cuando toca
     * reencuadrar de todos modos.
     */
    const mirando = chartRef.current?.timeScale().getVisibleRange() ?? null;
    ...
    /**
     * Lo que marca una publicación: el momento en que salió el dato, el final
     * del plazo que miden las cifras de al lado, y el precio de justo antes.
     *
     * Aquí y no en la creación del gráfico, al contrario que la entrada y la
     * salida de una operación, porque estas tres cosas sí cambian sin que
     * cambie el gráfico: el plazo se elige con un botón, y la referencia se
     * recalcula cada vez que llega un tramo de velas anteriores -- la última
     * vela previa a la publicación pasa a ser otra.
     *
     * Flechas y no rayas verticales porque es lo que la librería sostiene de
     * forma estable al ampliar y desplazar.
     */
    if (t0 !== null && chartRef.current) {
      const horizonte = evento?.horizonMinutes ?? 0;
      const paso = GRANULARITY_SECONDS[granularity];
      const velaDelDato = visibleCandles.find((c) => c.time + paso > t0)?.time ?? null;
      const finPlazo = t0 + horizonte * 60;
      const velaDelFin = visibleCandles.find((c) => c.time + paso > finPlazo)?.time ?? null;

      const marcas: SeriesMarker<Time>[] = [];
      if (velaDelDato !== null) {
        marcas.push({
          time: velaDelDato as UTCTimestamp,
          position: "aboveBar",
          shape: "arrowDown",
          color: THEME.exit,
          // Una palabra: el texto va centrado sobre la flecha y la flecha cae
          // cerca del borde izquierdo, así que «se publica» salía recortado a
          // «publica» en la pantalla de un móvil.
          text: "dato",
        });
      }
      ...
      if (horizonte > 0 && velaDelFin !== null && velaDelFin !== velaDelDato) {
        marcas.push({
          time: velaDelFin as UTCTimestamp,
          position: "belowBar",
          shape: "arrowUp",
          color: withAlpha(THEME.text, 0.8),
          text: formatHorizonLabel(horizonte),
        });
      }

      // La misma primitiva, actualizada. Llamar otra vez a `createSeriesMarkers`
      // apilaría una capa nueva sobre la anterior en cada refresco.
      if (marcadoresRef.current) marcadoresRef.current.setMarkers(marcas);
      else marcadoresRef.current = createSeriesMarkers(series, marcas);

      // Y la referencia. Se quita la anterior antes de poner la nueva: sin eso
      // se apilaba una por cada tramo que llegaba, y el eje acababa con tres
      // etiquetas «previo» superpuestas que además no eran el mismo precio.
      if (lineaPrevioRef.current) {
        series.removePriceLine(lineaPrevioRef.current);
        lineaPrevioRef.current = null;
      }
      const previo = precioAntesDe(visibleCandles, t0);
      if (previo !== null) {
        lineaPrevioRef.current = series.createPriceLine({
          price: previo,
          color: withAlpha(THEME.text, 0.7),
          lineWidth: 1,
          lineStyle: LineStyle.Dashed,
          axisLabelVisible: true,
          // Corto a propósito: el título se pinta *dentro* del área del
          // gráfico, pegado al eje, y «antes del dato» tapaba las últimas velas.
          title: "previo",
        });
      }
    }
    ...
    if (fittedGranularityRef.current !== fitKey) {
      if (t0 !== null) {
        // Una publicación no se encuadra a lo que hay -- se traen velas hasta
        // dos días después del dato y eso no es lo que se viene a mirar --
        // sino a un número de velas alrededor del momento. En velas y no en
        // minutos: es lo que hace que se vea igual de bien en un minuto que en
        // una hora.
        const encuadre = encuadreInicial(
          t0,
          GRANULARITY_SECONDS[granularity],
          visibleCandles.map((c) => c.time),
        );
    ...
    } else if (t0 !== null && mirando) {
      // Ni reencuadre ni desplazamiento: lo que había delante sigue delante,
      // por muchas velas que hayan llegado por los lados.
      //
      // Dos veces, y la segunda en el fotograma siguiente. La librería no
      // aplica los datos al instante: los encola y los vuelca en el siguiente
      // repintado, y ese volcado lleva su propio encuadre --el del borde
      // derecho-- que pisaba al de aquí. Puesto sólo una vez, el gráfico
      // acababa a veces sobre la publicación y a veces un día después, según
      // qué llegara antes.
      const escala = chartRef.current?.timeScale();
      escala?.setVisibleRange(mirando);
      cancelAnimationFrame(reencuadreRef.current);
      reencuadreRef.current = requestAnimationFrame(() => {
        // El gráfico puede haberse ido entre el fotograma de antes y el de
        // ahora; `timeScale()` de uno destruido revienta.
        try {
          chartRef.current?.timeScale().setVisibleRange(mirando);
        } catch {
          /* el gráfico ya no está */
        }
      });
    }
  ...
  useEffect(() => {
    ...
    function redraw() {
       ...
       */
      if (entry && showPlan && planLevels.stop !== null && planLevels.target !== null) {
        const planTool = direction === "LONG" ? "LONG_POSITION" : "SHORT_POSITION";
        const anclaje = entry;
        pintar(
          planTool,
          [
            { time: anclaje.time, price: anclaje.price },
            { time: anclaje.time, price: planLevels.stop },
            { time: anclaje.time, price: planLevels.target },
          ],
    ...
    return (
      <p className="py-8 text-center text-sm text-muted-foreground">
        {esOperacion
          ...
          : "No hay datos de velas disponibles de Coinbase para este momento."}
      </p>
    );
  }
    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-md border border-border bg-secondary/40 p-1">
            ...
            />
            <ToggleButton
              label={esOperacion ? "Restablecer vista" : "Volver a centrar en la publicación"}
              Icon={esOperacion ? Maximize2 : LocateFixed}
              pressed={false}
              onClick={resetView}
            />
            {/* Copiar los dibujos de la operación anterior del mismo producto
                sólo tiene sentido entre operaciones: lo que se repite de una a
                la siguiente son tus niveles, y una publicación de hace un mes
                no tiene «la anterior» con la que compartirlos. */}
            {esOperacion ? (
              <ToggleButton
            ...
            <ToggleButton
              label={
                esOperacion
                  ? "Reproducir la operación vela a vela"
                  : "Reproducir la reacción vela a vela"
              }
              Icon={Play}
              pressed={replaying}
              onClick={() => {
            ...
            />
            {/* La captura se guarda *en* las capturas de la operación, así que
                sin operación no tiene dónde ir. */}
            {esOperacion ? (
              <ToggleButton
      ...
      {replaying ? (
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-primary/40 bg-primary/5 px-3 py-2 text-sm">
          ...
          <p className="w-full text-xs text-muted-foreground">
            {esOperacion
              ? "La salida está oculta mientras dura la reproducción: se trata de leer el gráfico sin saber ya cómo terminó."
              : "Vela a vela, como se vio en directo: la gracia es decidir qué habrías hecho sin saber ya en qué acabó el movimiento."}
          </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          ...
          {evento
            ? ` · la raya de puntos es el precio de justo antes del dato, y las flechas marcan la publicación y el final del plazo de ${formatHorizonLabel(evento.horizonMinutes)} que miden las cifras · al llegar al borde se traen más velas`
            : ""}
        </p>
        {measurement ? (
          <button
