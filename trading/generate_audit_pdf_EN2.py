...
OUTPUT = r"C:\Users\pekas\OneDrive\Escritorio\claude\trading\BTC_DC_Strategy_Audit_FINAL.pdf"
...
C_YELLOW   = colors.HexColor("#9A6700")
C_BLUE     = colors.HexColor("#0969DA")
C_PURPLE   = colors.HexColor("#8250DF")
C_WHITE    = colors.HexColor("#1F2328")
C_MUTED    = colors.HexColor("#656D76")
C_HEADER_BG= colors.HexColor("#0969DA")
C_ROW_ALT  = colors.HexColor("#F6F8FA")
C_ACCENT   = colors.HexColor("#CF222E")
...
doc = SimpleDocTemplate(
    ...
    topMargin=2*cm,
    bottomMargin=2*cm,
    title="BTC DC Breakout L+S — Complete Strategy Audit",
...
S_CODE = sty("S_CODE",
    fontSize=7.5, leading=11, textColor=colors.HexColor("#0969DA"), spaceAfter=2,
    fontName="Courier", backColor=colors.HexColor("#F6F8FA"), leftIndent=8, rightIndent=8)

S_CODE_COMMENT = sty("S_CODE_COMMENT",
    fontSize=7.5, leading=11, textColor=colors.HexColor("#656D76"), spaceAfter=2,
    fontName="Courier", backColor=colors.HexColor("#F6F8FA"), leftIndent=8, rightIndent=8)

S_CODE_KW = sty("S_CODE_KW",
    fontSize=7.5, leading=11, textColor=colors.HexColor("#CF222E"), spaceAfter=2,
    fontName="Courier", backColor=colors.HexColor("#F6F8FA"), leftIndent=8, rightIndent=8)
...
def green(txt):
    return f'<font color="#1A7F37"><b>{txt}</b></font>'

def red(txt):
    return f'<font color="#CF222E"><b>{txt}</b></font>'

def blue(txt):
    return f'<font color="#0969DA"><b>{txt}</b></font>'

def yellow(txt):
    return f'<font color="#9A6700"><b>{txt}</b></font>'

def purple(txt):
    return f'<font color="#8250DF"><b>{txt}</b></font>'

def muted(txt):
    return f'<font color="#656D76">{txt}</font>'

def bold(txt):
    return f'<b>{txt}</b>'

def code(txt):
    return f'<font name="Courier" color="#0969DA">{txt}</font>'
...
def make_table(data, col_widths=None, header_bg=C_HEADER_BG, alt_bg=C_ROW_ALT,
               font_size=8.5, header_color=colors.HexColor("#FFFFFF")):
    ...
    def _wrap(cell, is_hdr):
        if isinstance(cell, str):
            ...
            st = ParagraphStyle("_ac", fontSize=fs, leading=fs + 3,
                                textColor=colors.HexColor("#FFFFFF") if is_hdr else C_WHITE,
...
toc_items = [
    ...
    ("4.", "Strategy Logic and Architecture", "6"),
    ("5.", "Optimization Process", "8"),
    ("6.", "Comparison: Long-Only vs Long+Short", "10"),
    ("7.", "Complete Backtest Results (L+S)", "11"),
    ("8.", "Detailed Analysis — LONG Trades", "13"),
    ("9.", "Detailed Analysis — SHORT Trades", "14"),
    ("10.", "Risk Analysis and Advanced Metrics", "16"),
    ("11.", "10-Year Perspective (BTC Daily)", "18"),
    ("12.", "Conclusions and Recommendations", "20"),
]
for num, title, pg in toc_items:
    row = Table([[
        Paragraph(f"<b><font color='#0969DA'>{num}</font></b>", S_TOC),
        Paragraph(f"<font color='#1F2328'>{title}</font>", S_TOC),
        Paragraph(f"<font color='#656D76'>{pg}</font>", ParagraphStyle("toc_pg", fontSize=9.5, leading=14, alignment=TA_RIGHT, textColor=C_MUTED, fontName="Helvetica")),
...
story.append(p(
    ...
    f"generated +122,914% vs +20,329% for Buy & Hold — {bold('6 times superior')}.", S_BODY))
...
# ══════════════════════════════════════════════════════════════════════════════
# 4. LOGIC AND ARCHITECTURE (was 5)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("4. STRATEGY LOGIC AND ARCHITECTURE", S_H1))
story.append(hr(C_BLUE))

story.append(p("4.1 LONG Entry Condition", S_H2))
...
story.append(p("4.2 SHORT Entry Condition", S_H2))
...
story.append(p("4.3 Exit System", S_H2))
story.append(p(
...
story.append(p("4.4 Key Architectural Decision: One-Way Filter", S_H2))
story.append(p(
...
story.append(p("4.5 Why dc_lower Uses CLOSES (not lows)", S_H2))
story.append(p(
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. OPTIMIZATION PROCESS (was 6)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("5. OPTIMIZATION PROCESS", S_H1))
story.append(hr(C_BLUE))

story.append(p("5.1 Phase 1 — Long Side Optimization", S_H2))
...
story.append(p("5.2 Phase 2 — Incorporating Shorts (problems and solutions)", S_H2))
...
story.append(p("5.3 Phase 3 — Short Parameter Optimization", S_H2))
...
story.append(p("5.4 What Did NOT Improve Results (All Tested and Rejected)", S_H2))
rejected = [
...
# ══════════════════════════════════════════════════════════════════════════════
# 6. COMPARISON LONG-ONLY vs L+S (was 7)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("6. COMPARISON: LONG-ONLY vs LONG+SHORT", S_H1))
story.append(hr(C_BLUE))
...
story.append(sp(8))
story.append(p("6.1 Complete Comparison Table", S_H2))
...
story.append(p("6.2 Analysis: Why Do Shorts IMPROVE Performance?", S_H2))
story.append(p(
...
# ══════════════════════════════════════════════════════════════════════════════
# 7. COMPLETE L+S RESULTS (was 8)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("7. COMPLETE BACKTEST RESULTS (L+S)", S_H1))
story.append(hr(C_BLUE))

story.append(p("7.1 Global Metrics", S_H2))
global_data = [
...
story.append(p("7.2 Risk-Adjusted Metrics", S_H2))
risk_adj = [
...
story.append(p("7.3 Trade Distribution", S_H2))
dist_data = [
...
# ══════════════════════════════════════════════════════════════════════════════
# 8. LONG ANALYSIS (was 9)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("8. DETAILED ANALYSIS — LONG TRADES", S_H1))
...
story.append(sp(6))
story.append(p("8.1 Entry Condition — Combined Filters", S_H2))
story.append(p(
...
story.append(p("8.2 Statistical Characteristics of Longs", S_H2))
long_stats = [
...
story.append(p("8.3 Why EMA(200) on 4H is the Optimal Filter for Longs", S_H2))
story.append(p(
...
# ══════════════════════════════════════════════════════════════════════════════
# 9. SHORT ANALYSIS (was 10)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("9. DETAILED ANALYSIS — SHORT TRADES", S_H1))
...
story.append(sp(6))
story.append(p("9.1 Short Logic: High Asymmetry, Low Frequency", S_H2))
story.append(p(
...
story.append(sp(4))
story.append(p("9.2 Statistical Characteristics of Shorts", S_H2))
short_stats = [
    ["METRIC", "VALUE", "INTERPRETATION"],
...
story.append(p("9.3 Short Activation Conditions — Analysis", S_H2))
story.append(p(
...
story.append(p("9.4 Comparison: Different Macro Filters for Shorts", S_H2))
macro_test = [
...
# ══════════════════════════════════════════════════════════════════════════════
# 10. RISK ANALYSIS (was 11)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("10. RISK ANALYSIS AND ADVANCED METRICS", S_H1))
story.append(hr(C_YELLOW, 1))

story.append(p("10.1 Drawdown — Complete Analysis", S_H2))
dd_data = [
...
story.append(sp(6))
story.append(p("10.2 Margin Usage and Required Capital", S_H2))
margin_data = [
    ["METRIC", "VALUE", "NOTE"],
...
story.append(p("10.3 Leverage Analysis", S_H2))
story.append(p(
...
story.append(sp(8))
story.append(p("10.4 Institutional Quality Ratios Summary", S_H2))
quality = [
...
# ══════════════════════════════════════════════════════════════════════════════
# 11. 10-YEAR BTC DAILY (was 12)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("11. 10-YEAR PERSPECTIVE — BTC DAILY (2016–2026)", S_H1))
story.append(hr(C_BLUE))
...
story.append(p("11.1 Important Context: Historical BTC Volatility", S_H2))
story.append(p(
    f"The extraordinary returns shown in the 10-year backtest (+122,914%) must be interpreted "
    f"in proper historical context. Bitcoin in the 2016–2020 period was a fundamentally "
    f"different asset than it is today: market capitalization was measured in the billions "
    f"(not hundreds of billions), liquidity was extremely thin, and a single large buyer "
    f"could move the price by 20-50% in days. The explosive moves captured by the strategy "
    f"— such as the 2017 rally from $800 to $20,000 (+2,400%) or the 2020-2021 bull run "
    f"from $10,000 to $69,000 (+590%) — reflect an era of {bold('hyperbolic volatility')} "
    f"that is progressively diminishing as institutional adoption increases.", S_BODY))
story.append(p(
    f"As Bitcoin matures into a recognized institutional asset class, the market dynamics "
    f"are shifting. {bold('The 2023–2026 backtest results (+537.91%) are a far more realistic ')} "
    f"{bold('benchmark for future performance')} than the 10-year figures. "
    f"The daily chart perspective serves primarily to validate that the strategy's core logic "
    f"— trend-following with volatility-adjusted stops — is {green('robust across different')} "
    f"{green('market regimes')}, not to project that 100,000%+ returns are achievable today.", S_BODY))
story.append(p(
    f"For current live trading expectations: the {yellow('~7.2%/month rate from the 4H backtest')} "
    f"(Dic 2023 – May 2026) provides a much more grounded projection, as it reflects "
    f"Bitcoin's behavior in the post-ETF, institutional-grade market environment.", S_BODY))

story.append(p("11.2 Interpretation of the 52.84% Drawdown", S_H2))
story.append(p(
...
# ══════════════════════════════════════════════════════════════════════════════
# 12. CONCLUSIONS (was 14)
# ══════════════════════════════════════════════════════════════════════════════
story.append(p("12. CONCLUSIONS AND RECOMMENDATIONS", S_H1))
story.append(hr(C_BLUE))

story.append(p("12.1 Main Findings", S_H2))
findings = [
    ["#", "KEY FINDING", "IMPACT"],
...
story.append(p("12.2 Recommendations for Live Trading", S_H2))
recs = [
...
story.append(p("12.3 Limitations and Risks", S_H2))
story.append(p(
